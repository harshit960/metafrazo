import React from 'react'

function CTAWideBtn() {
    return (
        <>
            <button 
            // style={{
            //     boxShadow: "0px 0px 25px 10px #3277F826",
            //     background: "linear-gradient(0deg, #0A47F2 79.79%, #B5C9FF 122.34%),radial-gradient(342.89% 359.97% at 49.74% 48.94%, rgba(10, 71, 242, 0.5) 0%, rgba(181, 201, 255, 0.5) 100%)"
            // }} 
            className="font-satoshi button w-[352px] h-[51px] mt-[32px] bg-[#0A47F2] rounded-[16px] text-[18px] text-white flex items-center justify-center font-bold">
                Get Started Now
            </button>

        </>)
}

export default CTAWideBtn